...
class TransitionIndicatorDot extends StatelessWidget {
  ...
  TransitionIndicatorDot({Key key, @required this.currentPageIndex, @required this.currentContainerIndex, this.radius = 8, this.duration}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      width: currentPageIndex == currentContainerIndex ? context.dynamicWidth((2*radius) / 412) : context.dynamicWidth(radius / 412),
      height: context.dynamicWidth(radius / 412),
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(context.dynamicWidth(radius/412)),
        color: currentPageIndex == currentContainerIndex ? Colors.grey.shade600 : Colors.grey.shade400,
      ),
    );
  }
}
